...
describe('Base32 Encoding and Decoding', () => {
  it('should encode and decode a string to base32', () => {
    ...
    const encoded = Base32.encode(Buffer.from(input), { type: 'rfc4648', alphabet: Base32.rfc4648.alphabet, lowercase: false })
    const decoded = Base32.decode(encoded.toString(), { type: 'rfc4648', charmap: Base32.rfc4648.charmap.toString() })
  ...
  it('should return a buffer from a string', () => {
    const decoder = new Base32.Decoder({ type: 'rfc4648', charmap: Base32.rfc4648.charmap.toString() })
    const input = 'Hello World!'
    const encoded = Base32.encode(Buffer.from(input), { type: 'rfc4648', alphabet: Base32.rfc4648.alphabet, lowercase: false })
  ...
  it('should encode and decode bytes to base32', () => {
    ...
    const encoded = Base32.encode(input, { type: 'rfc4648', alphabet: Base32.rfc4648.alphabet, lowercase: false })
    const decoded = Base32.decode(encoded.toString(), { type: 'rfc4648', charmap: Base32.rfc4648.charmap.toString() })
